...
from track_manager.metadata import (
    flag_for_review, 
    apply_metadata_csv,
    verify_library
)
...
class TestMetadataWorkflow:
    ...
    def test_flag_for_review_creates_csv(self, temp_output_dir, 
                                         create_test_audio_file, test_config):
        ...
        create_test_audio_file(test_file, artist="Artist", 
                              title="Song [Official Video]", format='mp3')
    ...
    def test_flag_for_review_appends_to_csv(self, temp_output_dir,
                                           create_test_audio_file, test_config):
    ...
    def test_apply_metadata_csv(self, temp_output_dir,
                                       create_test_audio_file, test_config):
        ...
        test_file = temp_output_dir / "test.mp3"
        create_test_audio_file(test_file, artist="Wrong Artist",
                              title="Wrong Title", format='mp3')
        
        # Create CSV with corrections
        with open(csv_path, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=[
                'file_path', 'current_artist', 'current_title',
                'suggested_artist', 'suggested_title', 'source_url', 'notes'
            ])
            writer.writeheader()
            writer.writerow({
        ...
        audio = MP3(str(test_file), ID3=None)
        assert audio.get('artist', [None])[0] == 'Correct Artist'
        assert audio.get('title', [None])[0] == 'Correct Title'
        
        # Verify CSV row was removed
        with open(csv_path, 'r') as f:
            reader = csv.DictReader(f)
            rows = list(reader)
            assert len(rows) == 0
    
    def test_apply_partial_corrections(self, temp_output_dir,
                                      create_test_audio_file, test_config):
        ...
        assert audio1.get('artist', [None])[0] == 'New Artist1'
        
        audio2 = MP3(str(file2))
        assert audio2.get('artist', [None])[0] == 'Artist2'  # Unchanged
    ...
    def test_dry_run_mode(self, temp_output_dir, create_test_audio_file, test_config):
        ...
        # Create test file
        test_file = temp_output_dir / "test.mp3"
        create_test_audio_file(test_file, artist="Original", 
                              title="Original", format='mp3')
        ...
        assert audio.get('artist', [None])[0] == 'Original'
    ...
    def test_verify_library(self, temp_output_dir, create_test_audio_file):
        ...
        create_test_audio_file(junk_file, artist="Artist",
                              title="Song [Official Video]", format='mp3')
    ...
    def test_csv_removed_when_empty(self, temp_output_dir,
                                   create_test_audio_file, test_config):
