...
CSV_HEADERS = ["file_path", "current_artist", "current_title", "suggested_artist",
               "suggested_title", "source_url", "notes"]
...
def show_pending_reviews(csv_path: Path):
    ...
    for i, row in enumerate(rows, 1):
        ...
        print(f"   Suggested: {row['suggested_artist'] or '(empty)'} - {row['suggested_title'] or '(empty)'}")
        if row['notes']:
